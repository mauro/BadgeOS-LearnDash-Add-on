...
function badgeos_learndash_step_etc_select( $step_id, $post_id ) {
    ...
    // Lesson Category
    echo '<select name="badgeos_learndash_lesson_category_id" class="select-lesson-category-id">';
    ...
    // Topic Category
    echo '<select name="badgeos_learndash_topic_category_id" class="select-topic-category-id">';
...
// With this add_action enabled, all sub-menus for learndash activities in
// badge editing screen appeared twice. Removing this fixed the issue.
...
function badgeos_learndash_save_step( $title, $step_id, $step_data ) {
    ...
    if ( 'learndash_trigger' == $step_data[ 'trigger_type' ] ) {
        ...
        elseif ( 'badgeos_learndash_topic_completed_tag' == $step_data[ 'learndash_trigger' ] ) {
            // Get Object ID
            $object_id = (int) $step_data[ 'learndash_topic_category_id' ];
            ...
            if ( empty( $object_id ) ) {
                ...
                $title = sprintf( __( 'Completed topic in tag "%s"', 'badgeos-learndash' ), get_term( $object_id, 'ld_course_tag' )->name );
            }
        }
        // Lesson Category specific
        elseif ( 'badgeos_learndash_lesson_completed_tag' == $step_data[ 'learndash_trigger' ] ) {
            // Get Object ID
            $object_id = (int) $step_data[ 'learndash_lesson_category_id' ];
            ...
            if ( empty( $object_id ) ) {
                ...
                $title = sprintf( __( 'Completed lesson in tag "%s"', 'badgeos-learndash' ), get_term( $object_id, 'ld_course_tag' )->name );
            }
        }
...
function badgeos_learndash_step_js() {
    ...
    <script type="text/javascript">
        jQuery( document ).ready( function ( $ ) {
            ...
            $( document ).on( 'change', '.select-trigger-type', function () {
                ...
                if ( 'learndash_trigger' == trigger_type.val() ) {
                    ...
                    var fields = ['quiz','lesson','course','course-category','lesson-category','topic-category'];
            ...
            // Trigger a change so we properly show/hide our LearnDash menus
            ...
            $( document ).on( 'update_step_data', function ( event, step_details, step ) {
                ...
                step_details.learndash_topic_category_id = $( '.select-topic-category-id', step ).val();
                step_details.learndash_lesson_category_id = $( '.select-lesson-category-id', step ).val();
        ...
        function badgeos_learndash_step_change( $this , times) {
            ...
            // Topic Category specific
            trigger_parent.find( '.select-topic-category-id' )
                .toggle( 'badgeos_learndash_topic_completed_tag' == trigger_value );

            // Lesson Category specific
            trigger_parent.find( '.select-lesson-category-id' )
            ...
            if ( ( 'learndash_quiz_completed' == trigger_value && '' != trigger_parent.find( '.select-quiz-id' ).val() )
                ...
                || ( 'badgeos_learndash_topic_completed_tag' == trigger_value && '' != trigger_parent.find( '.select-topic-category-id' ).val() )
                || ( 'badgeos_learndash_lesson_completed_tag' == trigger_value && '' != trigger_parent.find( '.select-lesson-category-id' ).val() )
